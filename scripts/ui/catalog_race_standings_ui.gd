...
func _process(_delta: float) -> void:
	if game == null or panel == null:
		return
	var should_show: bool = GameState.current_mode == GameState.MODE_RACE and game.player != null and is_instance_valid(game.player)
	...
	var player_position: int = maxi(1, game.race_progress.position_of(game.player))
	...
	for index in range(mini(5, order.size())):
		...
		if racer == null:
			continue
		var definition: Dictionary = catalog.get_vehicle(racer.vehicle_id)
		var display_name: String = str(definition.get("name", racer.vehicle_id))
		if display_name.length() > 12:
			display_name = display_name.left(12)
		var is_player: bool = racer == game.player
		var team_tag: String = "YOU" if is_player else _team_tag(racer)
		var prefix: String = ">" if is_player else " "
		var suffix: String = " FIN" if bool(standing.get("finished", false)) else " L%d" % maxi(0, int(standing.get("lap", 0)) + 1)
		lines.append("%s%d [%s] %s%s" % [prefix, index + 1, team_tag, display_name, suffix])
	list_label.text = "\n".join(lines)

func _team_tag(vehicle: ArcadeVehicle) -> String:
	var team_id: String = str(vehicle.get_meta("team_id", ""))
	if team_id.is_empty():
		return "RIVAL"
	var definition: Dictionary = team_catalog.team(team_id)
	var display_name: String = str(definition.get("display_name", team_id)).to_upper()
