...
func _test_backup_recovery() -> void:
	...
	save_manager.delete_track(track.track_id)
	_expect(save_manager.save_track(track), "initial track save succeeds")
	track.name = "Backup Version Two"
	track.dirty = true
	_expect(save_manager.save_track(track), "second track save succeeds and creates backup")
	...
	var recovered: TrackData = save_manager.load_track(track.track_id) as TrackData
	...
	if recovered != null:
		...
		_expect(save_manager.save_track(recovered), "recovered backup can safely repair primary")
		var repaired: TrackData = save_manager.load_track(track.track_id) as TrackData
...
func _test_future_schema_rejection() -> void:
	...
	var loaded: TrackData = save_manager.load_track_path(path) as TrackData
...
func _test_corrupt_import_rejection() -> void:
	...
	var imported: TrackData = save_manager.import_track(path) as TrackData
